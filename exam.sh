...
process_manage() {
    ...
    case $act in
        list_zombies)
            echo -e "${BLUE}Zombie process list:${NC}"
            ps -eo pid,stat,comm,user | grep 'Z' || echo "no zombie found"
            ;;
        list_stopped)
            echo -e "${BLUE}Stopped process list:${NC}"
            ps -eo pid,stat,comm,user | grep 'T' || echo "no stopped found"
            ;;
        kill_zombies)
            echo -e "${YELLOW}cant kill zombie directly bro, parent must handle${NC}"
            ;;
        kill_stopped)
            echo -e "${YELLOW}killing stopped ones...${NC}"
